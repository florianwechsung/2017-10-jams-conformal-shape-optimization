...
\begin{frame}
    ...
    \begin{prop}[Proposition (Decomposition)\cite{iglesias2017shape}]
        ...
        \end{equation}
        with $\ub_\alpha^{(1)}\in V$ and $\ub_\alpha^{(2)}\in V^\perp$.
...
\begin{frame}[b]{Weighted conformality}
    \begin{itemize}
        \item The method works well for cases in which there exists a conformal mapping between the initial and the optimal shape.
        \item Unfortunately:
    \end{itemize}
    \begin{prop}[Proposition \cite{iglesias2017shape} (but appears to be well-known)]
    ...
    \begin{challengeblock}{Problem}
        If the shape is fixed somewhere, then there are no non-trivial conformal mappings.
    \end{challengeblock}
...
\begin{frame}{Example: drag minimization}
    \begin{itemize}
        \item Consider an obstacle in a fluid. Goal: minimize the drag while keeping the volume constant.
        \item The boundary of the channel is fixed $\implies$ conformal mapping impossible.
        \item Pick $\mu$ large close to the obstacle to guarantee conformality there.
    \end{itemize}
    \begin{center}
